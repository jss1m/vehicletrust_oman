...
def main() -> int:
    ...
    if not (ROOT.parent / ".github" / "workflows" / "vehicletrust-ci.yml").is_file():
        failures.append("missing root GitHub Actions workflow")
    ...
    forbidden_tracked = [
        path
        for path in tracked
        if path.endswith(".env") or path.endswith(".db") or path.endswith(".pem")
    ]
